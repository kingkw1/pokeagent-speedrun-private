...
class OpenerBot:
    ...
    def _build_state_machine(self) -> Dict[str, BotState]:
        ...
        def action_special_clock(state_data: Dict[str, Any], visual_data: Dict[str, Any]) -> Optional[List[str]]:
            ...
            Strategy: Don't try to parse partial dialogue text (unreliable due to VLM timing).
            Instead, detect the Yes/No question by position and dialogue presence, then clear all dialogue.
            Returns None when all dialogue is cleared to allow transition to S8.
            """
            dialogue = visual_data.get('on_screen_text', {}).get('dialogue', '').upper()
            text_box_visible = visual_data.get('visual_elements', {}).get('text_box_visible', False)
            
            # Detect Yes/No question by checking for specific keywords (most reliable)
            if text_box_visible and "IS THIS" in dialogue and "TIME" in dialogue:
                logger.info("[CLOCK] Detected Yes/No question - selecting 'Yes' with UP")
                return ['UP', 'A']  # UP to select Yes, then A to confirm
            
            # Detect initial clock interaction by checking for CLOCK keyword
            if text_box_visible and "CLOCK" in dialogue:
                logger.info("[CLOCK] Detected clock interaction - pressing A")
                return ['A']
            
            # Clear any other dialogue at the clock position
            if text_box_visible:
                logger.info("[CLOCK] Clearing dialogue at clock - pressing A")
                return ['A']
            
            # No dialogue visible - clock sequence is complete
            logger.info("[CLOCK] Clock sequence complete - transitioning to S8")
            return None

        def action_exit_house(state_data: Dict[str, Any], visual_data: Dict[str, Any]) -> Union[List[str], NavigationGoal]:
            """Handles multi-floor navigation to exit the house.
            
            Phase 1: If on 2F, navigate to stairs at (8,2) and use them
            Phase 2: If on 1F, navigate to exit door at (4,7)
            
            Always clears any dialogue first.
            """
            # Clear dialogue if present
            if visual_data.get('visual_elements', {}).get('text_box_visible', False):
                return ['A']
            
            player_location = state_data.get('player', {}).get('location', '').upper()
            x = state_data.get('player', {}).get('position', {}).get('x', 0)
            y = state_data.get('player', {}).get('position', {}).get('y', 0)
            
            # Phase 1: On 2F - go to stairs
            if '2F' in player_location:
                # If already at stairs, use them (walk DOWN)
                if x == 8 and y == 2:
                    print("[EXIT HOUSE] At stairs (8,2), going DOWN")
                    return ['DOWN']
                # Otherwise navigate to stairs
                print(f"[EXIT HOUSE] Phase 1: At ({x},{y}), navigating to stairs (8,2)")
                return NavigationGoal(x=8, y=2, map_location='PLAYERS_HOUSE_2F', description="Go to Stairs (2F)")
            
            # Phase 2: On 1F - go to exit
            if '1F' in player_location:
                print(f"[EXIT HOUSE] Phase 2: At ({x},{y}), navigating to exit (4,7)")
                return NavigationGoal(x=4, y=7, map_location='PLAYERS_HOUSE_1F', description="Exit House (1F)")
            
            # Fallback - shouldn't reach here
            print(f"[EXIT HOUSE] Unexpected location: {player_location}")
            return None
        ...
        return {
            ...
            'S6_NAV_TO_CLOCK': BotState(
                ...
                milestone_check='PLAYER_NAME_SET',
                memory_check=lambda s: ('HOUSE 2F' in s.get('player', {}).get('location', '').upper() and
                                       # Only exclude exact clock position - let S7 handle it there
                                       not (s.get('player', {}).get('position', {}).get('x', 0) == 5 and 
                                            s.get('player', {}).get('position', {}).get('y', 0) == 1)),
            ...
            'S7_SET_CLOCK': BotState(
                ...
                milestone_check='PLAYER_NAME_SET',
                memory_check=lambda s: ('HOUSE 2F' in s.get('player', {}).get('location', '').upper() and
                                       s.get('player', {}).get('position', {}).get('x', 0) >= 4 and 
                                       s.get('player', {}).get('position', {}).get('x', 0) <= 6 and
                                       s.get('player', {}).get('position', {}).get('y', 0) >= 1 and 
                                       s.get('player', {}).get('position', {}).get('y', 0) <= 2),
                # Simply detect if ANY dialogue is visible at clock position
                # Don't try to parse partial text - just handle all dialogue with A
                visual_check=lambda v: v.get('visual_elements', {}).get('text_box_visible', False),
            ...
            'S8_NAV_OUT_OF_HOUSE': BotState(
                ...
                description='Navigate out of the house (2F → stairs → 1F → exit)',
                milestone_check='PLAYER_NAME_SET',
                memory_check=lambda s: 'HOUSE' in s.get('player', {}).get('location', '').upper() and 'LAB' not in s.get('player', {}).get('location', '').upper(),
                # Activate when NO dialogue visible (clock sequence complete)
                visual_check=lambda v: not v.get('visual_elements', {}).get('text_box_visible', False),
                # Multi-floor navigation: 2F → stairs, then 1F → exit
                action_fn=action_exit_house,  # Special handler for multi-floor exit
    ...
    def should_handle(self, state_data: Dict[str, Any], visual_data: Dict[str, Any]) -> bool:
        ...
        player_x = player_pos.get('x', -1)
        player_y = player_pos.get('y', -1)
        milestones = state_data.get('milestones', {})
        text_box_visible = visual_data.get('visual_elements', {}).get('text_box_visible', False)
        
        print(
            f"🤖 [OPENER BOT] should_handle() called - "
            f"loc: {player_location}, pos: ({player_x}, {player_y}), "
            f"text_box: {text_box_visible}, milestones: {list(milestones.keys())}"
        )
        ...
        if milestones.get('STARTER_CHOSEN', False):
            ...
            if 'ROUTE_101' in player_location.upper() or 'ROUTE' not in player_location.upper():
                print("🤖 [OPENER BOT] STARTER_CHOSEN milestone complete and outside lab - handing off to VLM")
                return False
        ...
        for state_name, state in self.states.items():
            ...
            if self._check_state_match(state, state_data, visual_data):
                if self.current_state_name != state_name:
                    print(f"🤖 [OPENER BOT] ✅ State detected: {state_name} - taking control")
                return True
        
        print(f"🤖 [OPENER BOT] ❌ No matching state found - VLM will take control")
        return False
    ...
    def _check_state_match(self, state: BotState, state_data: Dict[str, Any], 
        ...
        print(f"🤖 [OPENER BOT]   Checking {state.name}...")
        
        # Check milestone requirements (highest confidence)
        if state.milestone_check:
            milestone_met = milestones.get(state.milestone_check, False)
            print(f"🤖 [OPENER BOT]     milestone_check: {state.milestone_check}={milestone_met}")
            if not milestone_met:
                print(f"🤖 [OPENER BOT]     ❌ {state.name} failed: milestone {state.milestone_check} not met")
                return False
        
        if state.milestone_incomplete:
            milestone_present = milestones.get(state.milestone_incomplete, False)
            print(f"🤖 [OPENER BOT]     milestone_incomplete: {state.milestone_incomplete}={milestone_present}")
            if milestone_present:
                print(f"🤖 [OPENER BOT]     ❌ {state.name} failed: milestone {state.milestone_incomplete} already complete")
                return False
        
        # Check memory state (high confidence)
        if state.memory_check:
            try:
                memory_result = state.memory_check(state_data)
                print(f"🤖 [OPENER BOT]     memory_check: {memory_result}")
                if not memory_result:
                    print(f"🤖 [OPENER BOT]     ❌ {state.name} failed: memory_check returned False")
                    return False
        ...
        if state.visual_check:
            try:
                visual_result = state.visual_check(visual_data)
                print(f"🤖 [OPENER BOT]     visual_check: {visual_result}")
                if not visual_result:
                    print(f"🤖 [OPENER BOT]     ❌ {state.name} failed: visual_check returned False")
                    return False
            except Exception as e:
                logger.warning(f"[OPENER BOT] Visual check failed for {state.name}: {e}")
                return False
        
        print(f"🤖 [OPENER BOT]     ✅ {state.name} MATCHED!")
        return True
